...
properties = {
  ...
  prototypingMode: true, // adds stop and comment to insert tool
  toolChangeDelay: true // adds tool change delay
};
...
function onOpen() {
  ...
  if (programName) {
    ...
    var oFormat = createFormat({width:1, zeropad:true, decimals:0});
    if (programComment) {
  ...
  writeBlock(gMotionModal.format(0), gPlaneModal.format(17), gFormat.format(40),
  gFormat.format(49), gFormat.format(52), gFormat.format(80), gAbsIncModal.format(90));
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    retracted = true;
    writeBlock(gFormat.format(0), gFormat.format(49), gFormat.format(52), "Z" + xyzFormat.format(0)); // retract
        forceXYZ();
  }
  ...
  if (insertToolCall) {
    ...
    if (tool.number > 99) {
      ...
      // move table for tool change
      writeBlock(gFormat.format(52))
      writeBlock(gFormat.format(0) + xOutput.format(-500) + yOutput.format(0))

      // change tool
      writeBlock("T" + toolFormat.format(tool.number));
      writeBlock(mFormat.format(6));

      // insert tool manually for prototypes
      if (properties.prototypingMode && tool.number > 8) {
        writeBlock(mFormat.format(0))

        var comment = "Change tool! :" +
      ...
      // tool change delay
        ...
        writeBlock(gFormat.format(4) + "P4.");
      }

      // set proper work offset
      var wcsCode = (currentSection.workOffset - 1) % 6

      if (currentWorkOffset) {
        writeBlock(gFormat.format(currentWorkOffset))
      } else if (wcsCode > 0) {
        writeBlock(gFormat.format(54 + wcsCode))
      } else {
 ...
 //   );
  ...
  if (insertToolCall || retracted) {
    ...
    //    gMotionModal.format(0), xOutput.format(initialPosition.x), yOutput.format(initialPosition.y)
     // );
      writeBlock(gMotionModal.format(0), gFormat.format(43), xOutput.format(initialPosition.x), yOutput.format(initialPosition.y),
	  zOutput.format(initialPosition.z), hFormat.format(lengthOffset),  gFormat.format(54 + code),
	  sOutput.format(tool.spindleRPM), mFormat.format(tool.clockwise ? 3 : 4)
	  );
    } else {
      writeBlock(
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    case "drilling":
      writeBlock(
        gPlaneModal.format(17), gAbsIncModal.format(90), gCycleModal.format(81), gRetractModal.format(98),
    ...
    case "counter-boring":
      if (P > 0) {
        writeBlock(
          gPlaneModal.format(17), gAbsIncModal.format(90), gCycleModal.format(82), gRetractModal.format(98),
          getCommonCycle(x, y, z, cycle.retract),
    ...
    case "chip-breaking":
      ...
      writeBlock(
        gPlaneModal.format(17), gAbsIncModal.format(90), gCycleModal.format(73), gRetractModal.format(98),
    ...
    case "deep-drilling":
      writeBlock(
        gPlaneModal.format(17), gAbsIncModal.format(90), gCycleModal.format(83), gRetractModal.format(98),
        getCommonCycle(x, y, z, cycle.retract),
        "Q" + xyzFormat.format(cycle.incrementalDepth),
        conditional(P > 0, "P" + milliFormat.format(P)), // optional
        feedOutput.format(F)
    ...
    case "reaming":
      if (P > 0) {
        writeBlock(
          gPlaneModal.format(17), gAbsIncModal.format(90), gCycleModal.format(89), gRetractModal.format(98),
          getCommonCycle(x, y, z, cycle.retract),
          "P" + milliFormat.format(P), // not optional
          feedOutput.format(F),
        ...
        writeBlock(
          gPlaneModal.format(17), gAbsIncModal.format(90), gCycleModal.format(85), gRetractModal.format(98),
          getCommonCycle(x, y, z, cycle.retract),
          feedOutput.format(F),
          "E" + feedFormat.format(cycle.retractFeedrate)
        );
    ...
    case "stop-boring":
      if (P > 0) {
        ...
        writeBlock(
          gPlaneModal.format(17), gAbsIncModal.format(90), gCycleModal.format(86), gRetractModal.format(98),
          getCommonCycle(x, y, z, cycle.retract),
          feedOutput.format(F)
        );
      }
...
writeBlock(gFormat.format(0),gFormat.format(49),gFormat.format(52), "X-215.", "Y0",  "Z0");
forceXYZ();
